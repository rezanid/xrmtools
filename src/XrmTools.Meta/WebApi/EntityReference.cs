...
public class EntityReference
{
    ...
    public EntityReference(string uri)
    {
        ...
        int lastSlashPosition = uri.LastIndexOf('/') + 1;
        if (lastSlashPosition >= 0 && firstParen > lastSlashPosition && lastParen > firstParen)
        {
            SetName = uri[lastSlashPosition..firstParen];
            ...
            else
            {
                ...
                try
                {
                    KeyAttributes = uri[firstParen++..lastParen].Split([','], StringSplitOptions.RemoveEmptyEntries)
